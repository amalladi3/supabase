...
Adding Keyri QR login into your Supabase-based authentication system is a matter of sending the user's 

# Sign up for Keyri

First create a free account on the Keyri dashboard (<https://app.keyri.com>) and register a new servive. On Add Your Application, set a name and input the domain on which your app will eventually be deployed. You can create multiple applications in Keyri to account for your development, staging, and production environments.
...
Next, for the same login view, set up an event listener to pick up the session token that the iFrame emits when the QR code is scanned by your mobile app.
